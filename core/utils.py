...
def get_split(period):
    start_year = 1990
    end_year = 2020
    year_tag = [str(i) for i in range(start_year, end_year + 1)]
    year_split = [month for month in year_tag if period[0] <= month <= period[1]]
    ...
    month_split = [(month_tag[i], month_tag[i + 1]) for i in range(len(month_tag)) if
